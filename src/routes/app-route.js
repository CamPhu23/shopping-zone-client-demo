      ...
      {routes.map(route => {
        if (route.private) {
          if (route.permission === ADMIN_PERMISSION) {
            return (
              <Route key={route.path} path={route.path} element={<AdminPrivateRoutes requirePermission={ADMIN_PERMISSION} component={route.component} />} />
            )
          } else if (route.permission === CLIENT_PERMISSION) {
            return (
              <Route key={route.path} path={route.path} element={<ClientPrivateRoutes requirePermission={CLIENT_PERMISSION} component={route.component} />} />
            )
          }
        } else {
          return (
