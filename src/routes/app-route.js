      ...
      {routes.map(route => {
        if (route.private) {
          if (route.permission === ADMIN_PERMISSION) {
            return (
              <Route key={route.path} path={route.path} element={<AdminPrivateRoutes component={route.component} />} />
            )
          } else if (route.permission === CLIENT_PERMISSION) {
            return (
              <Route key={route.path} path={route.path} element={<ClientPrivateRoutes component={route.component} />} />
            )
          }
        } else {
          return (
